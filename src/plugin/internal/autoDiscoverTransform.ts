...
function containsRenderableReactNode(node: t.Node | null | undefined): boolean {
  ...
  while (stack.length > 0) {
    ...
    for (const visitorKey of visitorKeys) {
      const value = (nextNode as unknown as Record<string, unknown>)[visitorKey]
...
export function isSupportedSourceFile(id: string): boolean {
...
export function isFirstPartyModule(id: string): boolean {
...
function isInThirdPartyAllowlist(
  moduleId: string,
...
export function resolveAutoDiscoverScope(
  normalizedId: string,
  options: AutoDiscoverResolutionOptions,
  ...
  if (
    shouldSkipThirdPartyModule(normalizedId) &&
